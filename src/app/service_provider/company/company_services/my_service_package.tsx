  ...
  const renderServiceItem = ({ item }: any) => {
    return (
      <View style={tw`bg-white p-4 rounded-2xl mb-4 `}>
        ...
        {item?.package_detail_items?.length > 0 && (
          <View style={tw`pl-8 gap-2`}>
            {item?.package_detail_items.map((detail: any, index: number) => (
              <View
                key={detail?.id || index}
                style={tw`flex-row items-center gap-2 w-[88%]`}
              >
        ...
        {/* Delivery Time */}
        <View style={tw`flex-row justify-between items-center px-3 my-3 py-2`}>
        ...
        {/* Price */}
        <View
          style={tw`bg-primary w-full h-12 rounded-full flex-row justify-between items-center px-4 my-2`}
        >
          <Text style={tw`text-white font-semibold text-xl`}>Cost:</Text>
          ...
          >
            <View style={tw`flex-1 py-4`}>
              ...
              <View style={tw` h-14 rounded-lg  mt-1`}>
                <Dropdown
                  style={tw.style(
                    `h-14 rounded-lg px-4 bg-white border border-gray-300`,
                  )}
            ...
            <PrimaryButton
              ...
              loading={isLoadingCreateConnectAccount}
            />
          </BottomSheetScrollView>
        </BottomSheetModal>
      </BottomSheetModalProvider>
