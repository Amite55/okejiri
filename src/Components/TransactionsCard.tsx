import {
  IconProfileBadge,
  IconTransactionCredit,
  IconTransactionDebit,
} from "@/assets/icons";
  ...
  return (
    <View style={tw`flex-row items-center justify-between `}>
      <View style={tw`flex-row items-center gap-4`}>
        <SvgXml
          xml={type === "credit" ? IconTransactionCredit : IconTransactionDebit}
        />
        <View style={tw`gap-1`}>
          {varient === "withdraw" && (
            <Text
              style={tw`font-DegularDisplayDemoMedium text-xl text-[#FF3A00]`}
            >
              {title}
            </Text>
          )}
      ...
      <Text
        style={tw`font-DegularDisplayDemoMedium ${type === "credit" ? "text-[#00B230]" : "text-[#FF3A00]"}  text-lg `}
      >
        ₦ {price}
      </Text>
    </View>
