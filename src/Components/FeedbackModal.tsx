...
export default function FeedbackModel({ ref, id, onClose, onSubmit }: any) {
  ...
  const [selectedRatingsStatus, setSelectedRatingsStatus] = useState<
    string | null
  >(null);
        ...
        >
          <View>
            <View style={tw`items-center py-6 gap-3`}>
              ...
              <Text
                style={tw`font-DegularDisplayDemoSemibold text-2xl text-black`}
              >
                ₦ {orders?.price}
              </Text>
            </View>
              ...
              <View style={tw`flex-row  items-center gap-6 py-4`}>
                {[...Array(5)].map((_, index) => (
                  ...
                  >
                    <View>
                      {index < rating ? (
                        <SvgXml xml={IconratingRatedStar} />
                      ) : (
                        <SvgXml xml={IconratingUnratedStar} />
                      )}
                    </View>
                  </TouchableOpacity>
                ))}
